...
```bash
cd /Users/user/projects/second-brain
...
services:
  brain:
    ...
    volumes:
      - ${BRAIN_HOST_PATH:-~/Documents/MyVault}:/brain
    env_file:
      - .env
    stdin_open: true
    tty: true
...
- A `#` heading
- No references to Obsidian, MyVault, or specific vault paths

```bash
grep -r "Obsidian\|MyVault\|myvault\|/Users/user" skills/
```
