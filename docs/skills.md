...
- **Brain-local** (the profile's `skills/vault/`) — auto-installed by `brain-init` into `<brain>/.claude/skills/`. Load when Claude Code is opened at the brain root. Use MCP for semantic search and direct filesystem tools for file I/O.

Both tiers live in the active profile (the default is [brain-profile-ace](https://github.com/thekitchencoder/brain-profile-ace)); editing skills means editing them under the profile repo's `skills/{global,vault}/`.
...
Each skill's own `SKILL.md` under the profile repo's `skills/{global,vault}/` describes what it does.
