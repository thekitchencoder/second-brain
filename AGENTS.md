...
### UI image dev (adds code-server + Codex)
...
| `skills/`, `brain-skills/` | `task sync-skills` | copies to `~/.Codex/skills/`; run `/reload` in Codex |
...
Two tiers of Codex skills:
- **`skills/`** — Global skills (11): MCP-only, work from any host session via `Codex plugin add`. Includes brain-capture, brain-connect, brain-context, brain-create-effort, brain-distil, brain-effort, brain-project, brain-save, brain-setup, brain-surface, brain-triage.
...
Both tiers are baked into the Docker image and seeded into `~/.Codex/skills/` inside the container by `entrypoint.sh`. Global skills use MCP tools only; brain-local skills also use filesystem tools for structural operations. When editing skills, check both directories.

### Brain Structure Convention (ACE-aligned)

Notes are organized as `Atlas/`, `Efforts/`, `Cards/`, `Calendar/`, `Sources/`. Templates in `zk/templates/` define frontmatter schemas per note type. `brain-init` creates the brain scaffold including `.zk/`, `.ai/`, `.vscode/`, `.Codex/skills/`, and optionally the ACE folder structure.
...
| `Dockerfile.ui` | UI image — FROM base, adds code-server + Codex + extensions |
